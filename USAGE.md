...
cd rust
./target/debug/claw --model "gpt-4.1" prompt "reply with the word ready"
# or explicitly lock routing to Azure:
...
**Model-name prefix routing:** If a model name starts with `openai/`, `azure/`, `gpt-`, `qwen/`, or `qwen-`, the provider is selected by the prefix regardless of which env vars are set. This prevents accidental misrouting to Anthropic when multiple credentials exist in the environment.
