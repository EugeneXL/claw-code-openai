...
/// Accepts any non-empty, no-whitespace model/deployment identifier.
/// Provider-prefixed routes like `openai/gpt-4.1-mini` are still valid, but
/// bare OpenAI-compatible model names are also allowed so local gateways and
/// Azure deployment names do not get rejected before provider detection runs.
fn validate_model_syntax(model: &str) -> Result<(), String> {
    ...
    if trimmed.contains(' ') {
        return Err(format!(
            "invalid model syntax: '{}' contains spaces. Use a single model or deployment identifier",
            trimmed
        ));
    }
    Ok(())
...
"#,
    ...
    fn parses_single_word_command_aliases_without_falling_back_to_prompt_mode() {
        ...
        // OpenAI-compatible and Azure deployment names should parse without
        // requiring a provider prefix up front.
        let parsed_gpt = parse_args(&[
            "prompt".to_string(),
            "test".to_string(),
            "--model".to_string(),
            "gpt-4".to_string(),
        ])
        .expect("`--model gpt-4` should parse");
        match parsed_gpt {
            CliAction::Prompt { model, .. } => assert_eq!(model, "gpt-4"),
            other => panic!("expected prompt action, got {other:?}"),
        }

        let parsed_qwen = parse_args(&[
            "prompt".to_string(),
            "test".to_string(),
            "--model".to_string(),
            "qwen-plus".to_string(),
        ])
        .expect("`--model qwen-plus` should parse");
        match parsed_qwen {
            CliAction::Prompt { model, .. } => assert_eq!(model, "qwen-plus"),
            other => panic!("expected prompt action, got {other:?}"),
        }

        let parsed_azure = parse_args(&[
            "prompt".to_string(),
            "test".to_string(),
            "--model".to_string(),
            "my-azure-deployment".to_string(),
        ])
        .expect("Azure deployment name should parse");
        match parsed_azure {
            CliAction::Prompt { model, .. } => assert_eq!(model, "my-azure-deployment"),
            other => panic!("expected prompt action, got {other:?}"),
        }
    }
